...
<title>Goosta-report</title>
	...
	<link href="resources/css/report/report.css" rel="stylesheet" type="text/css" />
	...
	<div class="cont758">
		<div class="reportboard">
			...
			<table>
				...
				<c:choose>
					<c:when test="${fn:length(list)>0 }">
						...
						<tr>
							<td>${row.rn }</td>
							<td style="text-align: left;"><a href="reportDetail.do?boardno=${row.boardno}&curPage=${page.curPage}">${row.boardtitle}
					...
					<tr>
						<td colspan="5" style="background-color: #dbdbdb;">
							...
							<c:if test="${page.curPage > 1}">
								<a href="report.do?curPage=1&search_option=${search_option}&search=${search}">시작</a>
							</c:if>
							<!-- 이전  -->
							<c:if test="${page.curBlock > 1 }">
								<a href="report.do?curPage=${page.prevPage }&search_option=${search_option}&search=${search}"><</a>
							</c:if>
							<c:forEach var="pageNum" begin="${page.blockStart }" end="${page.blockEnd }">
								...
								<c:choose>
									...
									<c:otherwise>
										<a href="report.do?curPage=${pageNum }&search_option=${search_option}&search=${search}">
											${pageNum }&nbsp;&nbsp;
										</a>
									</c:otherwise>
								</c:choose>
							...
							<c:if test="${page.curBlock <= page.totBlock}">
								<a href="report.do?curPage=${page.nextPage }&search_option=${search_option}&search=${search}">></a>
							</c:if>
							<!-- 마지막 -->
							<c:if test="${page.curPage < page.totPage}">
								<a href="report.do?curPage=${page.totPage }&search_option=${search_option}&search=${search}">끝</a>
							</c:if>
						</td>
					</tr>
				</tfoot>
			...
			<div class="boardwrite">
				<a href="reportWrite.do" class="write"><span>글쓰기</span></a>
			</div>
